...
;; Met les valeurs des adresses liées à l'usage des bumpers à 0
INIT_BUMPERS_ADRESS
		MOV r0, #0
...
HANDLE_BUMPERS_INTERACTIONS
		...
		LDR r1, =RIGHT_BUMPER_ST_ADR    ; Charger l'adresse de RIGHT_BUMPER_ST_ADR dans r1
		LDR r2, [r1]    
		
		CMP r2, #1
		BNE CHECK_NEXT_BUMPER
		
		;; sinon éteint led et incrémente r8
		BL LED_DROITE_OFF;
		
		BL INCR_RIGHT_BUMPER_COUNTER
			
		;; délai de sécurité pour éviter le spam machine
		MOV r5, #1
		BL WAIT_R5_SECONDS
		
		;; allumage de la led pour indiquer que l'utilisateur peut de nouveau incrémenter
		BL LED_DROITE_ON;
		
CHECK_NEXT_BUMPER
		BL BUMPER_GAUCHE_READSTATE
		
		LDR r1, =LEFT_BUMPER_ST_ADR    ; Charger l'adresse de LEFT_BUMPER_ST_ADR dans r1
		LDR r2, [r1] 
...
BUMPER_GAUCHE_READSTATE
		LDR r2, =LEFT_BUMPER_ST_ADR ;; charge l'adresse dédiée à la récupération de l'état du bumper
...
BUMPER_DROIT_READSTATE
		LDR r2, =RIGHT_BUMPER_ST_ADR ;; charge l'adresse dédiée à la récupération de l'état du bumper
